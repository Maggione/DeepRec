...
from IO.iterator import FfmIterator, DinIterator, CCCFNetIterator, DKNIterator, RippleIterator, MKRIterator
from IO.din_cache import DinCache
from IO.ffm_cache import FfmCache
from IO.cccfnet_cache import CCCFNetCache
from IO.dkn_cache import DKNCache
from IO.Ripple_cache import RippleCache
from IO.mkr_cache import MKRCache
...
def create_train_model(model_creator, hparams, scope=None):
    ...
    with graph.as_default():
        ...
        if hparams.data_format == "mkr":
            src_dataset_kg = tf.data.TFRecordDataset(hparams.kg_file_cache)
        ...
        elif hparams.data_format == 'dkn':
            batch_input = DKNIterator(src_dataset, hparams)
        elif hparams.data_format == 'ripple':
            batch_input = RippleIterator(src_dataset, hparams)
        elif hparams.data_format == "mkr":
            batch_input = MKRIterator(src_dataset, src_dataset_kg)
        else:
...
# cache data
def cache_kg_data(hparams, filename):
    if hparams.data_format == "mkr":
        cache_obj = MKRCache()
    else:
        return
    hparams.kg_file_cache = util.convert_cached_name(filename, hparams.batch_size)
    cached_name = hparams.kg_file_cache
    print('cache filename:', filename)
    if not os.path.isfile(cached_name):
        print('has not cached file, begin cached...')
        start_time = time.time()
        cache_obj.write_kg_tfrecord(filename, cached_name, hparams)
...
def cache_data(hparams, filename, flag):
    ...
    if flag == 'train':
        ...
        sample_num_path = os.path.join(util.CACHE_DIR, util.TRAIN_NUM)
        impression_id_path = os.path.join(util.CACHE_DIR, util.TRAIN_IMPRESSION_ID)
    elif flag == 'eval':
        hparams.eval_file_cache = util.convert_cached_name(hparams.eval_file, hparams.batch_size)
        cached_name = hparams.eval_file_cache
        sample_num_path = os.path.join(util.CACHE_DIR, util.EVAL_NUM)
        impression_id_path = os.path.join(util.CACHE_DIR, util.EVAL_IMPRESSION_ID)
    elif flag == 'test':
        hparams.test_file_cache = util.convert_cached_name(hparams.test_file, hparams.batch_size)
        cached_name = hparams.test_file_cache
        sample_num_path = os.path.join(util.CACHE_DIR, util.TEST_NUM)
        impression_id_path = os.path.join(util.CACHE_DIR, util.TEST_IMPRESSION_ID)
    elif flag == 'infer':
        ...
        sample_num_path = os.path.join(util.CACHE_DIR, util.INFER_NUM)
        impression_id_path = os.path.join(util.CACHE_DIR, util.INFER_IMPRESSION_ID)
    else:
...
def train(hparams, scope=None, target_session=""):
    ...
    if hparams.kg_file is not None:
        cache_kg_data(hparams, hparams.kg_file)
    ...
    elif hparams.model_type == "ripple":
        print("run ripple network!")
        model_creator = RippleModel
    elif hparams.model_type == "mkr":
        print("run mkr network!")
        model_creator = MKRModel
    else:
        raise ValueError("model type should be cccfnet, deepFM, deepWide, dnn, fm, lr, ipnn, opnn, din, dkn, CIN, exDeepFM, ripple")
    ...
    if not hparams.infer_model_name is None and util.INFER == "infer":
        infer_path = hparams.infer_model_name
        try:
            train_model.model.saver.restore(train_sess, infer_path)
            print('load model', infer_path)
            if hparams.test_file is not None:
                test_res = run_eval(train_model, train_sess, hparams.test_file_cache,
                                    os.path.join(util.CACHE_DIR, util.TEST_NUM), hparams, flag='test')
                test_info = ', '.join(
                    [str(item[0]) + ':' + str(item[1]) for item in sorted(test_res.items(), key=lambda x: x[0])])
                print(' test info: ' + test_info)
            if hparams.infer_file is not None:
                run_infer(train_model, train_sess, hparams.infer_file_cache, hparams,
                            os.path.join(util.CACHE_DIR, util.INFER_NUM))
            return 0
        except:
            raise IOError("Failed to find any matching files for {0}".format(infer_path))
    ...
    print('total_loss = data_loss+regularization_loss, data_loss = {0}'.format(hparams.loss))
    step = 0
    ...
    for epoch in range(1, hparams.epochs + 1):
        ...
        if epoch % hparams.save_epoch == 0:
            save_model_dir = os.path.join(util.MODEL_DIR, 'epoch_' + str(epoch))
        ...
        train_res = run_eval(train_model, train_sess, hparams.train_file_cache, os.path.join(util.CACHE_DIR, util.TRAIN_NUM), hparams, flag='train')
        eval_res = run_eval(train_model, train_sess, hparams.eval_file_cache, os.path.join(util.CACHE_DIR, util.EVAL_NUM), hparams, flag='eval')
        train_info = ', '.join(
        ...
        if hparams.test_file is not None:
            test_res = run_eval(train_model, train_sess, hparams.test_file_cache, os.path.join(util.CACHE_DIR, util.TEST_NUM), hparams, flag='test')
            test_info = ', '.join(
                [str(item[0]) + ':' + str(item[1])
    ...
    if hparams.infer_file is not None:
        run_infer(train_model, train_sess, hparams.infer_file_cache, hparams, os.path.join(util.CACHE_DIR, util.INFER_NUM))
