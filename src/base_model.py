...
class BaseModel(object):
    def __init__(self, hparams, iterator, scope=None):
        ...
        self.loss = self._get_loss(hparams)
    ...
    def _train_opt(self, hparams, lr):
    ...
    def _build_train_opt(self, hparams):
        ...
        def train_opt(hparams):
            ...
            print(gradients)
            if hparams.is_clip_norm:
                gradients = [None if gradient is None else tf.clip_by_norm(gradient, hparams.max_grad_norm)
                             for gradient in gradients]
            train_op = train_step.apply_gradients(zip(gradients, variables))

            return train_op
        """
        train_step = self._train_opt(hparams, hparams.learning_rate)
        gradients, variables = zip(*train_step.compute_gradients(self.loss))
        if hparams.is_clip_norm:
            gradients = [None if gradient is None else tf.clip_by_norm(gradient, hparams.max_grad_norm)
                         for gradient in gradients]
    ...
    def _active_layer(self, logit, scope, activation, layer_idx=-1):
        if(layer_idx >= 0):
    ...
    # def print_tensor(self, sess):
    #     return sess.run(self.iterator.label, feed_dict={self.layer_keeps: self.keep_prob_test})
