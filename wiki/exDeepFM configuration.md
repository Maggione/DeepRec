...
model:
    ...
    model_type : exDeepFM => exDeepFM模型
    dim : 10 =>embedding的维度
    layer_sizes : [100, 100] => DNN使用的网络的结构
    activation : [relu, relu] => DNN网络结构中各层使用的激活函数
    dropout : [0.5, 0.5] => DNN网络结构中各层使用的dropout
    cross_layer_sizes : [100, 100, 50] => CIN中每一层的节点数
    infer_model_name : epoch_8 => infer阶段load的model
#    load_model_name : ./checkpoint/epoch_1 => 预训练好的模型
...
train:
    ...
    layer_l2 : 0.001 ==>DNN layer层参数的l2正则
    layer_l1 : 0.0000 ==>DNN layer层参数的l1正则
    cross_l2 : 0.0000 ==>CIN layer层参数的l2正则
    cross_l1 : 0.000 ==>CIN layer层参数的l1正则
